...
#ifdef ENABLE_NVMEMUL

void pnflush(void *addr, size_t size) {
    uint64_t *ptr;
...
void pflush_obj(void *addr) {
    pnflush(addr, sizeof(*addr));
}
...
void pkeyfree(void *key) {
    pfree(key, strlen(key) + 1);
}
...
void hashmap_base_clear(struct hashmap_base *hb)
{
    ...
    #ifdef ENABLE_NVMEMUL
    pnflush(hb->table, sizeof(struct hashmap_entry) * hb->table_size);
    #endif
}
...
void hashmap_base_reset(struct hashmap_base *hb)
{
    ...
    if (hb->table_size != hb->table_size_init) {
        #ifdef ENABLE_NVMEMUL
        size_t table_size = sizeof(struct hashmap_entry) * hb->table_size_init;
        new_table = (struct hashmap_entry*)prealloc(hb->table, sizeof(struct hashmap_entry) * hb->table_size, table_size);
        pnflush(new_table, table_size);
        #else
